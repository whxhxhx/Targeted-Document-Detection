...
def test(iter, logger, lrm, criterion, test_step=None, prefix='Test'):
    lrm.eval()
...
def train(iter, checkpoint_path, logger, fold, lrm, optimizer, criterion, epoch_num,
    ...
    for i in range(start_epoch, epoch_num):
        ...
        if val_iter:
            f1_score = test(iter=val_iter, logger=logger, lrm=lrm, prefix='Val',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='sbert_300', type=str)
    ...
    for i in range(kfold):
        if args.model_name in ['sbert_100', 'sbert_200', 'sbert_300']:
            lrm = SBERT_TDD(max_seq_length=args.max_seq_length, device=args.gpu)
            tokenizer = lrm.encoder.tokenizer
            device = lrm.encoder.device
        if args.model_name in ['DPR_100', 'DPR_200', 'DPR_300']:
            lrm = BiEncoder(max_seq_length=args.max_seq_length, device=args.gpu)
            tokenizer = lrm.encoder.tokenizer
            device = lrm.encoder.device
        ...
        train(train_iter, checkpoint_path, logger, i, lrm, opt, criterion, args.epochs, test_iter=test_iter,
