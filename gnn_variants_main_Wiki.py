...
def test(iter, logger, gim, lrm, criterion, test_step=None, prefix='Test'):
    gim.eval()
    lrm.eval()
...
def train(iter, checkpoint_path, logger, fold, gim, lrm, optimizer, criterion, epoch_num,
    ...
    for i in range(start_epoch, epoch_num):
        ...
        if val_iter:
            f1_score = test(iter=val_iter, logger=logger, gim=gim, lrm=lrm, prefix='Val',
    ...
    if test_iter:
        ...
        f1_score = test(iter=test_iter, logger=logger, gim=gim, lrm=lrm, prefix='Test',
                      criterion=criterion, test_step=i + 1)
...
if __name__ == '__main__':
    ...
    for i in range(kfold):
        ...
        if args.data_type == 'Wiki100':
            checkpoint_path = args.checkpoint_path + '/' + args.model_name + '_100'
        elif args.data_type == 'Wiki200':
            checkpoint_path = args.checkpoint_path + '/' + args.model_name + '_200'
        else:
            checkpoint_path = args.checkpoint_path + '/' + args.model_name + '_300'
        ...
        train(train_iter, checkpoint_path, logger, i, gim, lrm, opt, criterion, args.epochs, test_iter=test_iter,
